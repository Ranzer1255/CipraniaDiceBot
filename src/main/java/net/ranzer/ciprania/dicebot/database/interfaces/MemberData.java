...
import net.ranzer.ciprania.dicebot.data.IMemberData;
...
import net.ranzer.ciprania.dicebot.database.model.MemberPK;
	...
	public Map<Role, Long> getTimedRoles() {
		try (Session s = HibernateManager.getSessionFactory().openSession()) {
			...
			for (Map.Entry<String, Long> role : mdm.getTimedRoles().entrySet()) {
				rtn.put(
						CipraniaDiceBot.getJDA().getRoleById(role.getKey()),
						role.getValue());
			}
